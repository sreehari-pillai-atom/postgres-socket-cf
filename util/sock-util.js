...
const { socksAuthMessages } = require('./socks-messages')
...
class ConnectivitySocks {
  ...
  constructor() {
    xsenv.loadEnv()
    this.#connectivityCredentials = xsenv.serviceCredentials({
      label: "connectivity"
    }); //xsenv.cfServiceCredentials('connectivity')
  ...
  async #generateSocksClientOptions() {
    ...
    log("location length", iLocationLength);
      ...
      proxy: {
        ...
        custom_auth_request_handler: async () => {
          return Buffer.concat([
            Buffer.from([0x01]), // Authentication method version - currently 1 --old 0x01
        ...
        custom_auth_response_handler: async (data) => {
          ...
          if (data[1] === 0x00) {
            log("custom_auth_response_handler returning 'true'");
            return true
          } else {
            log("custom_auth_response_handler returning 'false'");
            return false
          }
        },
      },
